...
def example_1_load_all_coaches():
    ...
    for i, coach in enumerate(coaches, 1):
        print(
            f"{i:2d}. {coach.name:25s} ({coach.nationality:10s}) | {coach.primary_formation} | {coach.tactical_style}"
        )
...
def example_2_get_coach_profile():
    ...
    print(
        f"   ├─ Possession Preference:  {profile.possession_preference:.2f} (high = possession-based)"
    )
    print(
        f"   ├─ Pressing Intensity:     {profile.pressing_intensity:.2f} (high = aggressive pressing)"
    )
    print(f"   ├─ Width of Play:          {profile.width_of_play:.2f}")
    print(
        f"   └─ Transition Speed:       {profile.transition_speed:.2f} (high = fast breaks)"
    )
...
def example_3_coach_recommendations_for_state():
    ...
    recs = get_coach_recommendations_for_state(
        possession=35, fatigue=50, momentum=-2, score_differential=-2
    )
    ...
    recs = get_coach_recommendations_for_state(
        possession=65, fatigue=70, momentum=2, score_differential=1
    )
    ...
    recs = get_coach_recommendations_for_state(
        possession=50, fatigue=60, momentum=0, score_differential=0
    )
...
def example_5_formation_by_coach():
    ...
    from coaching.coaching_knowledge import get_all_coaches, get_formation_by_coach
...
def example_6_training_emphasis():
    ...
    print(
        f"{'Coach':<25} {'Aerobic':<10} {'Technical':<12} {'Tactical':<10} {'Mental':<10}"
    )
    print("-" * 67)
    ...
    for coach_name in coaches_to_compare:
        ...
        print(
            f"{coach_name:<25} {emphasis['aerobic']:<10.2f} {emphasis['technical']:<12.2f} "
...
def example_7_coaching_reward_calculation():
    ...
    # Get top coaches
    top_coaches = get_coach_recommendations_for_state(
        possession, fatigue, momentum, score_diff
    )[:3]
    ...
    print("  Base reward (tactic quality):    0.35")
    print("  Fatigue penalty:                -0.05")
    print("  Coaching bonus (3 coaches):     +0.15 (0.05 per coach alignment)")
    print(f"  {'─' * 50}")
    print("  Final Q-target:                  0.45  ✓ REINFORCED")
    print("\n  → AI learns that 'aggressive_press' is good for this state")
    print("  → Because elite pressing coaches (Klopp, Rose, Enrique) align with it")
