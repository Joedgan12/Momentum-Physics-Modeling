...
HEAD_SHA = sys.argv[1] if len(sys.argv) > 1 else subprocess.check_output(['git', 'rev-parse', 'HEAD']).decode().strip()
...
if __name__ == '__main__':
    ...
    prod_jobs = [j for j in jobs if 'montecarlo' in j['name'].lower() or 'production' in j['name'].lower()]
