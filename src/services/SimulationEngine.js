...
export class SimulationEngine {
  ...
  computePMU(player, eventHistory, crowdNoise = 75, timeElapsed = 45) {
    ...
    let baseEnergy = player.pos === 'GK' ? 8 : player.pos === 'DEF' ? 12 : player.pos === 'MID' ? 15 : 18
  ...
  simulateIteration(config) {
    ...
    const teamAPMUs = this.players.teamA.map(p => {
      const playerEvents = events.flatMap(e =>
    ...
    const pressureA = this.computePressure(this.players.teamA, this.players.teamB, coherence, tactic)
    const pressureB = this.computePressure(this.players.teamB, this.players.teamA, coherence, { possession: 1 / tactic.possession, offBall: 1 / tactic.offBall, transition: 1 })
  ...
  runScenarioSimulation(config) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      results.peakPMU = Math.max(results.peakPMU, Math.max(...iter.teamAPMUs), Math.max(...iter.teamBPMUs))
