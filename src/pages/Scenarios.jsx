  ...
  if (view === 'detail' && activeScenario) {
    return (
      <div className="page-container">
        ...
        <div style={{ background: 'var(--surface-1)', borderRadius: 12, border: '1px solid var(--surface-0-3)', padding: 20, marginBottom: 24 }}>
          <h2 style={{ color: 'var(--text-primary)', marginBottom: 8, fontSize: 24, fontWeight: 700 }}>
            {activeScenario.name}
          </h2>
          <p style={{ color: 'var(--text-secondary)', marginBottom: 12 }}>{activeScenario.description}</p>

          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
            <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 10 }}>
            ...
            <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 10 }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--team-a)', textTransform: 'capitalize' }}>
        ...
        {/* Key Metrics Summary */}
        <h3 style={{ color: 'var(--text-primary)', marginBottom: 12, fontSize: 18, fontWeight: 600 }}>
          Simulation Results
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 24 }}>
          {activeScenario.results && (
            <>
              <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>Team A Momentum</div>
              ...
              <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>Team B Momentum</div>
              ...
              <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>Expected Goals (xG)</div>
                <div style={{ fontSize: 24, fontWeight: 700, color: 'var(--plasma)' }}>
                  {activeScenario.results.xg?.toFixed(3) || '--'}
                </div>
              </div>
              <div style={{ background: 'var(--surface-0)', border: '1px solid var(--surface-0-3)', borderRadius: 8, padding: 12 }}>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>Goal Probability</div>
  ...
  if (view === 'comparison' && activeComparison) {
    return (
      <div className="page-container">
        ...
        <h2 style={{ color: 'var(--text-primary)', marginBottom: 20, fontSize: 24, fontWeight: 700 }}>
          {activeComparison.name}
        </h2>

        <div style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid var(--surface-0-3)', marginBottom: 24 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {activeComparison.scenarios.map((scenario) => {
                ...
                return (
                  <tr key={scenario.id} style={{ borderBottom: '1px solid var(--surface-0-3)', background: 'var(--surface-0)' }}>
                    <td style={{ padding: 12, fontWeight: 500, color: 'var(--text-primary)' }}>{scenario.name}</td>
                    <td style={{ padding: 12, textAlign: 'center', color: 'var(--text-secondary)' }}>{scenario.formation_a}</td>
                    <td style={{ padding: 12, textAlign: 'center', textTransform: 'capitalize', color: 'var(--text-secondary)' }}>
                      {scenario.tactic_a}
                    </td>
                    <td style={{ padding: 12, textAlign: 'center', fontWeight: 600, color: 'var(--team-a)' }}>
                      {scenario.results?.avgPMU_A?.toFixed(1) || '--'}
                    </td>
                    <td style={{ padding: 12, textAlign: 'center', fontWeight: 600, color: 'var(--plasma)' }}>
  ...
  return (
    <div className="page-container">
      ...
      {selectedScenarios.size > 0 && (
        <div className="panel" style={{ background: 'rgba(0, 229, 160, 0.08)', borderColor: 'var(--plasma)', marginBottom: '18px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <strong style={{ color: 'var(--plasma)', fontSize: '13px', fontWeight: '700' }}>{selectedScenarios.size} scenario(s) selected</strong>
            </div>
            <button
              className="btn-run"
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: 16, marginBottom: 20, borderBottom: '2px solid var(--surface-0-3)' }}>
        <button
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
              {scenarios.map((scenario) => (
                <div
                  key={scenario.id}
                  style={{
                    border: selectedScenarios.has(scenario.id) ? '2px solid var(--plasma)' : '1px solid var(--surface-0-3)',
                    borderRadius: 12,
                    padding: 16,
                    background: selectedScenarios.has(scenario.id) ? 'rgba(0, 229, 160, 0.06)' : 'var(--surface-1)',
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                  }}
                >
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12, lineHeight: 1.5 }}>
                    <div>
                      <strong>Formation:</strong> {scenario.formation_a} vs {scenario.formation_b}
                    </div>
                    <div>
