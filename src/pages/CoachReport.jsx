      ...
      <div className="page-container">
        <div className="command-header">
          <div className="command-identity">
            <div className="cmd-label"><Award size={14} style={{ display: 'inline', marginRight: '6px' }} /> Executive Brief</div>
            <h1>Coach Report</h1>
    ...
    <div className="page-container">
      ...
      <div className="command-header">
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '20px', fontFamily: 'var(--font-mono)', fontWeight: '800', color: 'var(--plasma)', marginBottom: '6px' }}>
            {simResults.elapsed_seconds != null ? `${simResults.elapsed_seconds.toFixed(2)}s` : '—'}
          </div>
          <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Elapsed</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '18px' }}>
        {[
          ...
          { label: 'Goal Probability', value: simResults.goalProbability ? `${(simResults.goalProbability * 100).toFixed(1)}%` : '—' },
          { label: 'Team A Win %', value: simResults.outcomeDistribution?.teamA_wins ? `${(simResults.outcomeDistribution.teamA_wins * 100).toFixed(1)}%` : '—', color: 'var(--team-a)' },
        ].map((stat, idx) => (
      ...
      <div className="panel" style={{ marginBottom: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
          {[
            { label: 'Team A Wins', value: simResults.outcomeDistribution?.teamA_wins, color: 'var(--team-a)' },
            { label: 'Draws', value: simResults.outcomeDistribution?.draws, color: 'var(--violet)' },
            { label: 'Team B Wins', value: simResults.outcomeDistribution?.teamB_wins, color: 'var(--team-b)' },
          ].map((item, idx) => (
            <div key={idx} style={{
              padding: '12px',
              ...
              <div style={{ fontSize: '24px', fontWeight: '800', color: item.color, fontFamily: 'var(--font-mono)' }}>
      ...
      {rolloutResults && (
        <div className="panel" style={{ marginBottom: '18px', borderColor: 'var(--success)', borderWidth: '1px' }}>
          <div className="panel-header">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            <div style={{ padding: '12px', borderRadius: 'var(--panel-radius)', background: 'var(--surface-0)' }}>
              <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px', fontWeight: '700', textTransform: 'uppercase' }}>
                Team A Momentum
              </div>
              <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--team-a)', fontFamily: 'var(--font-mono)' }}>
            ...
            <div style={{ padding: '12px', borderRadius: 'var(--panel-radius)', background: 'var(--surface-0)' }}>
              ...
              <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--team-b)', fontFamily: 'var(--font-mono)' }}>
            ...
            <div style={{ padding: '12px', borderRadius: 'var(--panel-radius)', background: 'var(--surface-0)' }}>
              ...
              <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--plasma)', fontFamily: 'var(--font-mono)' }}>
            ...
            <div style={{ padding: '12px', borderRadius: 'var(--panel-radius)', background: 'var(--surface-0)' }}>
              ...
              <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--success)', fontFamily: 'var(--font-mono)' }}>
                {(rolloutResults.confidence * 100)?.toFixed(0)}%
              </div>
            </div>
          </div>
          <div style={{ marginTop: '12px', fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
            Simulated {rolloutResults.iterations.toLocaleString()} iterations from current match state
          </div>
        </div>
      )}

      {rolloutError && (
        <div className="panel" style={{ marginBottom: '18px', borderColor: 'var(--warning)', borderWidth: '1px', background: 'var(--surface-error-light)' }}>
          <div className="panel-header">
            <div className="panel-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
