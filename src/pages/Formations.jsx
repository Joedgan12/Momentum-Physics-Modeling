...
const PRESET_FORMATIONS = {
  '4-3-3': {
    ...
    coherence: 0.87, style: 'Versatile',
  },
  '4-4-2': {
  ...
  '3-5-2': {
    ...
    coherence: 0.85, style: 'Attacking',
  },
  '5-3-2': {
    ...
    coherence: 0.86, style: 'Defensive',
  },
  '4-2-3-1': {
    ...
    coherence: 0.86, style: 'Structured',
  },
  '4-1-4-1': {
    ...
    coherence: 0.83, style: 'Defensive',
  },
  '4-3-2-1': {
  ...
  '3-4-3': {
    ...
    coherence: 0.80, style: 'Aggressive',
  },
  '4-2-4': {
    ...
    coherence: 0.78, style: 'Ultra-attack',
  },
}
...
function FormationPitch({ formation }) {
  ...
  return (
    <div style={{ position: 'relative', width: `${pitchW}px`, height: `${pitchH}px`, margin: '0 auto' }}>
      {/* pitch background */}
      <svg width={pitchW} height={pitchH} style={{ position: 'absolute', top: 0, left: 0 }}>
        <rect width={pitchW} height={pitchH} rx="6" fill="#1a3a0e" />
        {/* stripes */}
        {Array.from({ length: 5 }).map((_, i) => (
          <rect key={i} x={0} y={i * 64} width={pitchW} height={32}
            fill="rgba(255,255,255,0.03)" />
        ))}
        {/* centre circle */}
        <circle cx={pitchW / 2} cy={pitchH / 2} r={28} fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
        {/* half-way line */}
        <line x1={0} y1={pitchH / 2} x2={pitchW} y2={pitchH / 2} stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
        {/* goal areas */}
        <rect x={pitchW / 2 - 30} y={2} width={60} height={20} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="1" />
        <rect x={pitchW / 2 - 30} y={pitchH - 22} width={60} height={20} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="1" />
        {/* border */}
        <rect x={1} y={1} width={pitchW - 2} height={pitchH - 2} rx="5" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
      </svg>
      {/* player dots */}
      {players.map((p, i) => (
        <div key={i} style={{
    ...
    <div style={{ textAlign: 'center' }}>
      <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto 6px' }}>
        ...
        <div style={{
          ...
          fontFamily: 'var(--font-mono)', fontSize: '16px', fontWeight: '800', color,
        }}>{pct}%</div>
      </div>
      <div style={{ fontSize: '10px', color: 'var(--text-muted)', textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>Coherence</div>
    </div>
  )
}
...
function CustomBuilder({ onApply }) {
  ...
  const isValid = total === 10 && lines.length >= 2 && lines.length <= 5 && lines.every(l => l >= 1 && l <= 6)
  const formationStr = lines.join('-')

  const addLine = () => { if (lines.length < 5) setLines([...lines, 1]) }
  const removeLine = (i) => { if (lines.length > 2) setLines(lines.filter((_, idx) => idx !== i)) }
  ...
  return (
    <div className="panel" style={{ marginBottom: '18px' }}>
      <div className="panel-header">
        ...
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '14px', color: 'var(--plasma)', fontWeight: 800 }}>
          {formationStr}
        </div>
      </div>
      ...
      <div style={{ marginBottom: '14px' }}>
        <div style={{ fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: 8, textTransform: 'uppercase' }}>
        ...
        {lines.map((count, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <div style={{ width: 36, fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase' }}>
                {label}
              </div>
              <button
                onClick={() => changeLine(i, -1)}
                style={{ width: 26, height: 26, borderRadius: '50%', border: '1px solid var(--border-subtle)', background: 'var(--surface-0)', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              ><Minus size={12} /></button>
              <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 18, width: 24, textAlign: 'center', color: 'var(--text-primary)' }}>
                {count}
              </div>
              <button
                onClick={() => changeLine(i, +1)}
              ...
              <div style={{ display: 'flex', gap: 3, flex: 1 }}>
                {Array.from({ length: count }).map((_, j) => (
                  <div key={j} style={{ width: 10, height: 10, borderRadius: '50%', background: 'var(--plasma)', opacity: 0.8 }} />
                ))}
              </div>
              {lines.length > 2 && (
                <button onClick={() => removeLine(i)}
                  style={{ marginLeft: 'auto', background: 'none', border: 'none', color: 'var(--danger)', cursor: 'pointer', padding: 4 }}>
                  <Minus size={12} />
                </button>
              )}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, opacity: 0.5 }}>
          <div style={{ width: 36, fontSize: '10px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textTransform: 'uppercase' }}>GK</div>
          <div style={{ width: 26, height: 26 }} />
          <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 18, width: 24, textAlign: 'center' }}>1</div>
          <div style={{ width: 26, height: 26 }} />
      ...
      {lines.length < 5 && (
        <button onClick={addLine} style={{
          ...
          background: 'var(--surface-0)', border: '1px dashed var(--border-subtle)',
          borderRadius: 'var(--panel-radius)', color: 'var(--text-muted)', cursor: 'pointer',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
          Outfield players:{' '}
          <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, color: total === 10 ? 'var(--success)' : 'var(--danger)' }}>
            {total}
          </span>
          <span style={{ color: 'var(--text-muted)' }}> / 10</span>
        </div>
        {validationMsg && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '11px', color: 'var(--warning)' }}>
      ...
      <button
        ...
        disabled={!isValid}
        style={{
          width: '100%', padding: '10px', fontWeight: 800, fontSize: 13,
          ...
          borderRadius: 'var(--panel-radius)', cursor: isValid ? 'pointer' : 'not-allowed',
...
export default function Formations({ selectedFormation, onFormationChange, simResults }) {
  ...
  function computeLocalCoherence(formation) {
    ...
    try {
      ...
      return Math.max(0.70, Math.min(0.92, 0.88 - 0.015 * Math.abs(n - 3) - variance * 0.018))
    } catch { return 0.82 }
  }
    ...
    <div className="page-container">
      ...
      <div className="command-header">
        <div className="command-identity">
          <div className="cmd-label"><Send size={14} style={{ display: 'inline', marginRight: '6px' }} /> Tactical Setup</div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
        {['presets', 'custom'].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            ...
            borderRadius: 'var(--panel-radius)', cursor: 'pointer', textTransform: 'capitalize',
          }}>
      ...
      {tab === 'presets' && (
        <>
          ...
          <div style={{ marginBottom: '18px' }}>
            <div className="section-title" style={{ marginBottom: '12px', paddingLeft: '0' }}>Select Formation</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '8px' }}>
              {presetList.map(formation => (
                <button key={formation} onClick={() => onFormationChange(formation)} style={{
                  padding: '12px 8px',
                  background: selectedFormation === formation ? 'var(--plasma)' : 'var(--surface-1)',
                  color: selectedFormation === formation ? 'var(--void)' : 'var(--text-primary)',
                  border: `1px solid ${selectedFormation === formation ? 'var(--plasma)' : 'var(--border-subtle)'}`,
                  borderRadius: 'var(--panel-radius)', cursor: 'pointer',
                  fontWeight: '700', fontSize: '13px', fontFamily: 'var(--font-mono)',
                  transition: 'all 0.12s ease',
                }}>
                  {formation}
                  <div style={{ fontSize: '9px', marginTop: 4, opacity: 0.7, fontFamily: 'sans-serif' }}>
              ...
              {!isPreset && (
                <button style={{
                  padding: '12px 8px', background: 'var(--plasma)', color: 'var(--void)',
                  border: '1px solid var(--plasma)', borderRadius: 'var(--panel-radius)',
                  fontWeight: '700', fontSize: '13px', fontFamily: 'var(--font-mono)',
                }}>
                  {selectedFormation}
                  <div style={{ fontSize: '9px', marginTop: 4, opacity: 0.7, fontFamily: 'sans-serif' }}>Custom</div>
                </button>
              )}
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 18, marginBottom: 18 }}>
            <div className="panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px 12px' }}>
            ...
            <div>
              {currentDetails ? (
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <div className="panel" style={{ borderLeftWidth: '3px', borderLeftColor: 'var(--plasma)' }}>
                    <div className="section-title" style={{ marginBottom: '12px', color: 'var(--plasma)' }}>Strengths</div>
                    <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '12px', lineHeight: '1.7', color: 'var(--text-secondary)' }}>
                      {currentDetails.strengths.map((s, idx) => <li key={idx}>{s}</li>)}
                    </ul>
                  </div>
                  <div className="panel" style={{ borderLeftWidth: '3px', borderLeftColor: 'var(--danger)' }}>
                    <div className="section-title" style={{ marginBottom: '12px', color: 'var(--danger)' }}>Weaknesses</div>
                    <ul style={{ margin: 0, paddingLeft: '20px', fontSize: '12px', lineHeight: '1.7', color: 'var(--text-secondary)' }}>
                      {currentDetails.weaknesses.map((w, idx) => <li key={idx}>{w}</li>)}
                    </ul>
                  </div>
                </div>
              ) : (
                <div className="panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', flexDirection: 'column', gap: 8 }}>
                  <CheckCircle size={24} style={{ color: 'var(--success)' }} />
                  <div style={{ fontWeight: 700, fontSize: 14 }}>Custom formation applied</div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    Coherence score: <strong style={{ color: 'var(--plasma)' }}>{Math.round(coherence * 100)}%</strong>
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          ...
          {/* Comparison table */}
          <div className="panel">
            <div className="panel-header"><div className="panel-title">Formation Comparison</div></div>
            <div style={{ overflowX: 'auto' }}>
              <table className="player-table">
                ...
                <tbody>
                  {presetList.map(f => (
                    <tr key={f} onClick={() => onFormationChange(f)} style={{ cursor: 'pointer' }}>
                      <td>
                      ...
                      <td style={{ textAlign: 'center', fontSize: '11px', textTransform: 'capitalize' }}>
                        {PRESET_FORMATIONS[f].style}
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <code style={{ fontWeight: '700', color: PRESET_FORMATIONS[f].coherence >= 0.85 ? 'var(--success)' : 'var(--plasma)' }}>
      ...
      {tab === 'custom' && (
        <>
          ...
          <div className="panel" style={{ padding: '14px 16px', fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.7 }}>
            <strong style={{ color: 'var(--text-primary)' }}>Custom formation rules:</strong><br />
            • Outfield players must total <strong>10</strong> (GK is always added automatically)<br />
            • Each line must have between <strong>1 – 6</strong> players<br />
            • Min <strong>2 lines</strong>, max <strong>5 lines</strong><br />
            • Coherence is computed automatically from line balance
          </div>
        </>
      )}
