    ...
    <div className="page-container">
      ...
      <div className="command-header">
        <div className="command-identity">
          <div className="cmd-label">Match Breakdown</div>
          <h1>Match Analysis</h1>
          <p>{selectedFormation} vs 4-4-2 · {selectedTactic} vs balanced</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '12px', marginBottom: '18px' }}>
        <div className="stat-card">
          ...
          <div style={{ fontSize: '9px', color: 'var(--plasma)', fontFamily: 'var(--font-mono)', marginTop: '3px' }}>PMU</div>
        </div>
        <div className="stat-card">
          <div className="stat-card-label">Goal Probability</div>
          <div className="stat-card-value">{((simResults.goalProbability || 0.015) * 100).toFixed(1)}%</div>
          <div style={{ fontSize: '9px', color: 'var(--plasma)', fontFamily: 'var(--font-mono)', marginTop: '3px' }}>per 30s</div>
        </div>
        <div className="stat-card">
          <div className="stat-card-label">Expected Goals</div>
          <div className="stat-card-value">{(simResults.xg || 0.04).toFixed(3)}</div>
          <div style={{ fontSize: '9px', color: 'var(--plasma)', fontFamily: 'var(--font-mono)', marginTop: '3px' }}>xG</div>
        </div>
        <div className="stat-card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '14px', marginBottom: '18px' }}>
        {/* Momentum Comparison */}
        <div className="panel">
          <div className="panel-title" style={{ marginBottom: '12px' }}>Team Momentum Comparison</div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={momentumData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
              ...
              <Tooltip contentStyle={{ background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', color: 'var(--text-primary)' }} />
        ...
        {/* Expected Goals */}
        <div className="panel">
          <div className="panel-title" style={{ marginBottom: '12px' }}>Expected Goals (xG)</div>
          <ResponsiveContainer width="100%" height={280}>
        ...
        {outcomeData.length > 0 && (
          <div className="panel">
            <div className="panel-title" style={{ marginBottom: '12px' }}>Match Outcome Distribution</div>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
                ...
                >
                  {outcomeData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={['var(--team-a)', 'var(--violet)', 'var(--team-b)'][index]} />
                  ))}
                </Pie>
                <Tooltip formatter={(value) => `${value}%`} contentStyle={{ background: 'var(--surface-1)', border: '1px solid var(--border-subtle)' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        )}
      ...
      <div className="panel" style={{ marginBottom: '18px' }}>
        <div className="panel-title" style={{ marginBottom: '12px' }}>Pressure Streams</div>
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={pressureData} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
            ...
            <YAxis stroke="var(--text-muted)" style={{ fontSize: '12px' }} label={{ value: 'Pressure %', angle: -90, position: 'insideLeft' }} />
            <Tooltip contentStyle={{ background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', color: 'var(--text-primary)' }} />
            <Legend />
        ...
        <div className="panel" style={{ marginBottom: '18px' }}>
          ...
          <div>
            {simResults.playerMomentum.slice(0, 10).map((player, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 0', borderBottom: i < 9 ? '1px solid var(--border-subtle)' : 'none' }}>
                <div style={{ width: '24px', fontWeight: '800', color: 'var(--plasma)', fontFamily: 'var(--font-mono)', textAlign: 'right' }}>
                  #{i + 1}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: '600', color: 'var(--text-primary)', fontSize: '12px' }}>{player.name}</div>
                  <div style={{ fontSize: '10px', color: 'var(--text-muted)' }}>{player.position}</div>
                </div>
                <div style={{ width: '100px', height: '20px', background: 'var(--surface-0)', borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ width: `${(player.pmu / 50) * 100}%`, height: '100%', background: 'linear-gradient(90deg, var(--plasma), var(--pulse))' }} />
                </div>
                <div style={{ fontFamily: 'var(--font-mono)', fontWeight: '700', color: 'var(--plasma)', width: '40px', textAlign: 'right' }}>
      ...
      {/* Footer */}
      <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textAlign: 'center', padding: '14px', borderTop: '1px solid var(--border-subtle)' }}>
        <span>Iterations: {simResults.iterations || 500}</span>
        <span style={{ margin: '0 8px' }}>•</span>
        <span>Execution: {simResults.elapsed_seconds ? simResults.elapsed_seconds.toFixed(2) + 's' : '--'}</span>
        <span style={{ margin: '0 8px' }}>•</span>
