...
export default function Statistics({ simResults }) {
      ...
      <div className="page-container">
        ...
        <div className="command-header">
          <div className="command-identity">
            <div className="cmd-label"><TrendingUp size={14} style={{ display: 'inline', marginRight: '6px' }} /> Deep Dive</div>
  ...
  const stats = [
    ...
    { label: 'Goal Prob', value: (simResults.goalProbability * 100).toFixed(1) + '%' || '—', color: 'var(--flare)' },
    ...
    <div className="page-container">
      ...
      <div className="panel" style={{ marginBottom: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px' }}>
          <div style={{ textAlign: 'center', padding: '12px', borderRadius: 'var(--panel-radius)', background: 'rgba(59, 130, 246, 0.1)', borderLeft: '3px solid var(--team-a)' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px' }}>Team A Wins</div>
          ...
          <div style={{ textAlign: 'center', padding: '12px', borderRadius: 'var(--panel-radius)', background: 'rgba(139, 92, 246, 0.1)', borderLeft: '3px solid var(--violet)' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px' }}>Draws</div>
            <div style={{ fontSize: '20px', fontWeight: '800', color: 'var(--violet)', fontFamily: 'var(--font-mono)' }}>
              {simResults.outcomeDistribution?.draws != null
                ? `${(simResults.outcomeDistribution.draws * 100).toFixed(1)}%`
                : '—'}
            </div>
          </div>
          <div style={{ textAlign: 'center', padding: '12px', borderRadius: 'var(--panel-radius)', background: 'rgba(244, 114, 182, 0.1)', borderLeft: '3px solid var(--team-b)' }}>
            <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '6px' }}>Team B Wins</div>
      ...
      <div className="panel">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', fontSize: '12px', lineHeight: '1.8' }}>
          <div>
            <div style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <span style={{ float: 'right', fontWeight: '700', color: 'var(--plasma)', fontFamily: 'var(--font-mono)' }}>
                {simResults.avgPossession != null ? `${simResults.avgPossession.toFixed(1)}%` : '—'}
              </span>
            </div>
            <div style={{ marginBottom: '10px', paddingBottom: '10px', borderBottom: '1px solid var(--border-subtle)' }}>
              <span style={{ color: 'var(--text-muted)' }}>Avg Team Fatigue:</span>
              <span style={{ float: 'right', fontWeight: '700', color: 'var(--plasma)', fontFamily: 'var(--font-mono)' }}>
                {simResults.avgTeamFatigue != null ? `${simResults.avgTeamFatigue.toFixed(1)}` : '—'}
              </span>
            </div>
            <div>
              <span style={{ color: 'var(--text-muted)' }}>Crowd Noise:</span>
              <span style={{ float: 'right', fontWeight: '700', color: 'var(--plasma)', fontFamily: 'var(--font-mono)' }}>
                {simResults.crowd_noise_level != null ? `${simResults.crowd_noise_level.toFixed(0)} dB` : '—'}
              </span>
            </div>
          </div>
          <div>
