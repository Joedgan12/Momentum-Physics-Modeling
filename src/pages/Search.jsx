    ...
    <div className="page-container">
      ...
      <div className="command-header">
        <div className="command-identity">
          <div className="cmd-label"><Search size={14} style={{ display: 'inline', marginRight: '6px' }} /> Discovery</div>
          <h1>Player Search</h1>
      ...
      {filtered.length > 0 ? (
        <div style={{
          ...
          {filtered.map((player, idx) => {
            const hasSimilarNames = similarNamesMap[player.name] && similarNamesMap[player.name].length > 0
            ...
            <div key={idx} className="panel">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                {hasSimilarNames && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', color: 'var(--warning-color, #ff9800)', fontWeight: '600' }}>
                    <AlertCircle size={13} /> Similar
                  </div>
                )}
              </div>
              {hasSimilarNames && (
                <div style={{ fontSize: '9px', color: 'var(--text-muted)', marginBottom: '8px', padding: '6px 8px', background: 'rgba(255, 152, 0, 0.1)', borderRadius: '4px', borderLeft: '2px solid var(--warning-color, #ff9800)' }}>
                  <div style={{ fontWeight: '600', color: 'var(--warning-color, #ff9800)', marginBottom: '2px' }}>Similar names:</div>
                  {similarNamesMap[player.name].map((similar, i) => (
                    <div key={i}>{similar.similarTo}</div>
                  ))}
                </div>
              )}
              <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '10px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div>
                  <div style={{ color: 'var(--text-muted)', marginBottom: '2px' }}>Position</div>
                  <div style={{ fontWeight: '700', color: 'var(--text-primary)' }}>{player.position || '—'}</div>
                </div>
                <div>
                  <div style={{ color: 'var(--text-muted)', marginBottom: '2px' }}>Team</div>
                  <div style={{ fontWeight: '700', color: player.team === 'A' ? 'var(--team-a)' : 'var(--team-b)' }}>
                    {player.team === 'A' ? 'A' : 'B'}
                  </div>
                </div>
              </div>
              <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '8px' }}>
                <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '3px' }}>Momentum</div>
                <div style={{ fontSize: '16px', fontWeight: '800', color: 'var(--plasma)', fontFamily: 'var(--font-mono)' }}>
