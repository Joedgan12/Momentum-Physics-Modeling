...
export default function Match3D({ simResults, selectedFormation, selectedTactic }) {
  ...
  useEffect(() => { totalRef.current     = totalFrames }, [totalFrames])
  useEffect(() => { playbackRef.current  = playbackData },[playbackData])
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 7; i++) {
      const stripe = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0x2d5a3d : 0x336644, transparent: true, opacity: 0.5, depthWrite: false })
      )
    ...
    addLine([-52.5, 34, 0.02], [52.5, 34, 0.02], [52.5, -34, 0.02], [-52.5, -34, 0.02], [-52.5, 34, 0.02])
    ...
    ;[[0, 0, 0.3], [-41.5, 0, 0.2], [41.5, 0, 0.2]].forEach(([x, y, r]) => {
      const d = new THREE.Mesh(new THREE.CircleGeometry(r, 12), dotMat)
      d.position.set(x, y, 0.02); scene.add(d)
    })

    // Goals
    const goalMat = new THREE.MeshStandardMaterial({ color: 0xdddddd, metalness: 0.8, roughness: 0.3 })
    ;[-1, 1].forEach(side => {
      const gx = side * 52.5
      ;[-3.66, 3.66].forEach(gy => {
        const post = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 2.44, 8), goalMat)
        post.position.set(gx, gy, 1.22); post.rotation.x = Math.PI / 2; scene.add(post)
      })
      const bar = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 7.32, 8), goalMat)
      bar.position.set(gx, 0, 2.44); bar.rotation.z = Math.PI / 2; scene.add(bar)
    })
    ...
    for (let i = 0; i < 11; i++) {
      const pa = new THREE.Mesh(playerGeo, new THREE.MeshStandardMaterial({ color: TEAM_A_COLOR, metalness: 0.3, roughness: 0.4 }))
      pa.castShadow = true; pa.userData = { team: 'A', number: i + 1 }
      teamAGroup.add(pa); playersRef.current.teamA.push(pa)

      const pb = new THREE.Mesh(playerGeo, new THREE.MeshStandardMaterial({ color: TEAM_B_COLOR, metalness: 0.3, roughness: 0.4 }))
      pb.castShadow = true; pb.userData = { team: 'B', number: i + 1 }
      teamBGroup.add(pb); playersRef.current.teamB.push(pb)
    }
    scene.add(teamAGroup); scene.add(teamBGroup)

    const ball = new THREE.Mesh(
      new THREE.SphereGeometry(0.35, 16, 16),
      new THREE.MeshStandardMaterial({ color: 0xffffff, metalness: 0.4, roughness: 0.3 })
    )
    ball.castShadow = true; ball.position.set(0, 0, 0.35); scene.add(ball)
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowRight': setCurrentFrame(p => Math.min(totalRef.current - 1, p + 1)); break
        case 'ArrowLeft':  setCurrentFrame(p => Math.max(0, p - 1)); break
        case 'r': case 'R': setCurrentFrame(0); setIsPlaying(false); break
        case '+': case '=': setSpeed(s => Math.min(8, parseFloat((s * 2).toFixed(2)))); break
        case '-': case '_': setSpeed(s => Math.max(0.25, parseFloat((s / 2).toFixed(2)))); break
        case 'h': case 'H': setShowHeatmap(v => !v); break
        case '1': setCameraView('angled'); break
        case '2': setCameraView('top'); break
        case '3': setCameraView('side'); break
        case '4': setCameraView('goal'); break
        default: break
      }
    }
  ...
  const handleCanvasClick = useCallback((e) => {
    ...
    if (hits.length > 0) {
      ...
      allPlayers.forEach(p => { if (p.material.emissive) p.material.emissive.set(0x000000); p.scale.setScalar(1) })
  ...
  if (!simResults) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', flexDirection: 'column', gap: 16 }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: 0 }}>Run a simulation on the Overview tab to view 3D match analysis</p>
    </div>
  )
  ...
  const btnStyle = (active) => ({
    ...
    borderRadius: 'var(--panel-radius)', cursor: 'pointer', transition: 'all 0.12s',
  })
      ...
      <div className="dashboard-body">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <h2 style={{ margin: 0, fontSize: 18, color: 'var(--text-primary)' }}>3D Match Simulation</h2>
            <p style={{ margin: '4px 0 0', color: 'var(--text-muted)', fontSize: 12 }}>
              {selectedFormation} vs 4-4-2 &nbsp;·&nbsp; {selectedTactic} vs balanced
              &nbsp;·&nbsp;
              <kbd style={{ background: 'var(--surface-0)', padding: '1px 4px', borderRadius: 3, fontSize: 10 }}>Space</kbd> play
              &nbsp;
              <kbd style={{ background: 'var(--surface-0)', padding: '1px 4px', borderRadius: 3, fontSize: 10 }}>←→</kbd> step
              &nbsp;
              <kbd style={{ background: 'var(--surface-0)', padding: '1px 4px', borderRadius: 3, fontSize: 10 }}>H</kbd> heatmap
              &nbsp;
              <kbd style={{ background: 'var(--surface-0)', padding: '1px 4px', borderRadius: 3, fontSize: 10 }}>1-4</kbd> camera
            </p>
          </div>

          {/* 3D Canvas */}
          <div style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border-subtle)', marginBottom: 12 }}>
            {loading && (
              <div style={{ position: 'absolute', inset: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', flexDirection: 'column', gap: 10 }}>
            ...
            {selectedPlayer && (
              <div style={{
                ...
                <div style={{ fontWeight: 800, fontSize: 13, color: selectedPlayer.team === 'A' ? '#667eea' : '#f093fb', marginBottom: 6 }}>
                  Team {selectedPlayer.team} &nbsp;·&nbsp; #{selectedPlayer.number}
                </div>
                {[['Position', `(${selectedPlayer.x}, ${selectedPlayer.y})`], ['Speed', selectedPlayer.speed !== 'N/A' ? `${selectedPlayer.speed} m/s` : 'N/A'], ['Stamina', selectedPlayer.stamina !== 'N/A' ? `${selectedPlayer.stamina}%` : 'N/A']].map(([k, v]) => (
                  <div key={k} style={{ display: 'flex', justifyContent: 'space-between', gap: 14, fontSize: 11, color: '#ccc', marginBottom: 2 }}>
                    <span>{k}</span>
                ...
                <button
                  ...
                  style={{ marginTop: 8, fontSize: 10, background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 0 }}
                >
                  ✕ Deselect
                </button>
              </div>
          ...
          <div className="panel" style={{ padding: '12px 16px', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <button style={btnStyle(false)} onClick={() => { setCurrentFrame(0); setIsPlaying(false) }}>⏮</button>
              <button style={btnStyle(false)} onClick={() => setCurrentFrame(p => Math.max(0, p - 1))}>◀</button>
              <button style={{ ...btnStyle(isPlaying), minWidth: 76 }} onClick={() => setIsPlaying(p => !p)}>
                {isPlaying ? '⏸ Pause' : '▶ Play'}
              </button>
              <button style={btnStyle(false)} onClick={() => setCurrentFrame(p => Math.min(totalFrames - 1, p + 1))}>▶</button>
              ...
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
                {timeLabel} &nbsp;
                <span style={{ color: 'var(--text-secondary)' }}>f{currentFrame}/{totalFrames}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                />
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, width: 34 }}>{speed}x</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 14 }}>
            {[
              ...
              <div key={label} className="panel" style={{ textAlign: 'center', padding: '10px 8px' }}>
                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4, textTransform: 'uppercase', fontFamily: 'var(--font-mono)' }}>{label}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 18, color: 'var(--plasma)' }}>{val}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            {[
              { label: 'Team A', color: '#667eea', pmu: simResults.avgPMU_A, xg: simResults.xg_a, win: simResults.probability_outcomes?.team_a_win_probability },
              { label: 'Team B', color: '#f093fb', pmu: simResults.avgPMU_B, xg: simResults.xg_b, win: simResults.probability_outcomes?.team_b_win_probability },
            ].map(t => (
              <div key={t.label} className="panel" style={{ borderTopWidth: 3, borderTopColor: t.color }}>
                <div style={{ fontWeight: 800, color: t.color, marginBottom: 8, fontSize: 13 }}>{t.label}</div>
                {[
        ...
        {/* ── Right sidebar ── */}
        <div style={{ width: '300px', minWidth: '260px', maxWidth: '320px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="panel" style={{ padding: '12px 14px' }}>
            ...
            {[['#667eea', 'Team A (Blue)'], ['#f093fb', 'Team B (Pink)'], ['#ffffff', 'Ball']].map(([c, l]) => (
              <div key={l} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, fontSize: 12 }}>
                <div style={{ width: 14, height: 14, borderRadius: '50%', background: c, border: '1px solid rgba(255,255,255,0.3)', flexShrink: 0 }} />
            ...
            <div style={{ marginTop: 12, fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.8 }}>
              ...
              🖱 <strong>Click player</strong> to inspect stats<br />
              ⌨ <strong>Space</strong> &nbsp;·&nbsp; <strong>←→</strong> &nbsp;·&nbsp; <strong>H</strong> &nbsp;·&nbsp; <strong>±</strong> speed
            </div>
          </div>
        </div>
