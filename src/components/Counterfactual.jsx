    ...
    <div className="dashboard-center">
      ...
      <div className="hero-banner">
        <div className="hero-text">
          ...
          <p>Systematically test all formation × tactic combinations vs. a fixed opponent. Identify best tactical adjustments.</p>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            {/* Rank By */}
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', paddingLeft: 12, borderLeft: '1px solid #e5e7eb' }}>
              <div style={{ fontSize: 12, color: '#6b7280', fontWeight: 500 }}>Rank By</div>
              <select value={rankBy} onChange={(e) => setRankBy(e.target.value)} style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #d1d5db', fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', paddingLeft: 12, borderLeft: '1px solid #e5e7eb' }}>
              ...
              <select
                ...
                style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #d1d5db', fontWeight: 500 }}
              >
          ...
          {sweepError && (
            <div style={{ background: '#FEE2E2', border: '1px solid #FCA5A5', borderRadius: 8, padding: 12, color: '#991B1B', fontSize: 14, marginBottom: 12 }}>
              ❌ {sweepError}
            </div>
          )}
        </div>
      ...
      {/* Results */}
      {!sweepResults ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: '#f9fafb', borderRadius: 12, marginTop: 20 }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>📊</div>
          <div style={{ fontSize: 16, color: '#6b7280' }}>Run a sweep to see ranked tactical options</div>
        </div>
      ) : (
        <>
          {/* Summary Stats */}
          <div className="stats-row" style={{ marginTop: 20 }}>
            <div className="stat-card">
              <div className="stat-card-header">
                ...
                <div className="stat-icon" style={{ background: 'rgba(102,126,234,0.1)' }}>⚙️</div>
              </div>
            ...
            <div className="stat-card">
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span className="stat-value">{sweepResults.top_3_recommendations[0]?.formation}</span>
                <span className="stat-delta positive">{sweepResults.top_3_recommendations[0]?.tactic}</span>
              </div>
            </div>
            ...
            <div className="stat-card">
              <div className="stat-card-header">
                ...
                <div className="stat-icon" style={{ background: 'rgba(249,158,11,0.1)' }}>📋</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
            ...
            <div className="stat-card">
              <div className="stat-card-header">
                ...
                <div className="stat-icon" style={{ background: 'rgba(240,147,251,0.1)' }}>⏱️</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          ...
          <div style={{ marginTop: 24 }}>
            <h3 style={{ color: '#1f2937', marginBottom: 12, fontSize: 18, fontWeight: 600 }}>🏆 Top 3 Recommendations</h3>
              ...
              {sweepResults.top_3_recommendations.map((scenario, idx) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div>
                      <div style={{ fontSize: 20, fontWeight: 700, color: '#1f2937' }}>#{scenario.rank}</div>
                      <div style={{ fontSize: 14, color: '#6B7280' }}>
                        {scenario.formation} + <span style={{ fontWeight: 600, color: '#667EEA' }}>{scenario.tactic}</span>
                      </div>
                    </div>
                  ...
                  <div style={{ background: '#fff', borderRadius: 8, padding: 12, marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      ...
                      <span style={{ fontWeight: 600, color: '#1f2937' }}>{scenario.metrics.xg.toFixed(3)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontWeight: 600, color: '#1f2937' }}>{scenario.metrics.momentum_pmu.toFixed(1)} PMU</span>
                    </div>
                  </div>

                  <div style={{ fontSize: 12, color: '#6b7280', background: '#fff', borderRadius: 8, padding: 8 }}>
                    <div style={{ marginBottom: 4 }}>
                      <strong>Primary Action:</strong> {scenario.recommendations?.[0]?.action || 'No immediate action'}
                    </div>
          ...
          {/* Full Ranked Table */}
          <div style={{ marginTop: 24 }}>
            <h3 style={{ color: '#1f2937', marginBottom: 12, fontSize: 18, fontWeight: 600 }}>📊 Full Ranking (All {sweepResults.total_combinations} Combinations)</h3>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#667EEA', color: '#fff' }}>
                    ...
                    <th style={{ padding: 12, textAlign: 'center', fontWeight: 600 }}>Risk Level</th>
                  </tr>
                </thead>
                <tbody>
                  {sweepResults.ranked_scenarios.map((scenario) => (
                    <tr
                      key={scenario.combo}
                      style={{
                        background: scenario.rank <= 3 ? '#F0FDF4' : scenario.rank > 13 ? '#FEE2E2' : '#f9fafb',
                        borderBottom: '1px solid #e5e7eb',
                      }}
                    >
                      <td style={{ padding: 12, fontWeight: 600, color: '#667EEA' }}>#{scenario.rank}</td>
                      <td style={{ padding: 12, fontWeight: 500 }}>{scenario.formation}</td>
                      <td style={{ padding: 12, color: '#6b7280', textTransform: 'capitalize' }}>{scenario.tactic}</td>
                      <td style={{ padding: 12, textAlign: 'center', fontSize: 13, fontWeight: 600 }}>
          ...
          {sweepResults.concerning_scenarios && sweepResults.concerning_scenarios.length > 0 && (
            <div style={{ marginTop: 24, background: '#FEF2F2', border: '2px solid #FCA5A5', borderRadius: 12, padding: 16 }}>
              <h3 style={{ color: '#991B1B', marginBottom: 12, fontSize: 16, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {sweepResults.concerning_scenarios.map((scenario) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>
                      <strong>xG:</strong> {scenario.metrics.xg.toFixed(3)} ({formatDelta(scenario.metrics.xg_delta)})
                    </div>
          ...
          {/* Metadata */}
          <div style={{ marginTop: 16, padding: 12, background: '#f9fafb', borderRadius: 8, fontSize: 12, color: '#6b7280', textAlign: 'center' }}>
            Ranked by: <strong>{rankBy.toUpperCase().replace('_', ' ')}</strong> • Iterations per combo: <strong>{sweepResults.iterations_per_combo}</strong> • Request ID: <strong>{sweepResults.request_id}</strong>
          </div>
        </>
      )}
    </div>
