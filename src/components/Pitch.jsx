...
import { shortenPlayerName, getPlayerAbbreviation, detectSimilarNames } from '../utils/playerNameUtils'
...
const DEMO_PLAYERS = [
  ...
  { id: 'A6', name: 'Rodri Hernandez', number: 16, team: 'A', x: 38,  y: 34, pmu: 30, position: 'DM' },
  { id: 'A7', name: 'Phil Foden', number: 47, team: 'A', x: 48,  y: 22, pmu: 34, position: 'CM' },
  { id: 'A8', name: 'Kevin De Bruyne', number: 17, team: 'A', x: 48,  y: 46, pmu: 32, position: 'CM' },
  { id: 'A9', name: 'Jack Grealish', number: 10, team: 'A', x: 68,  y: 14, pmu: 38, position: 'LW' },
  { id: 'A10', name: 'Bernardo Silva', number: 20, team: 'A', x: 68,  y: 54, pmu: 36, position: 'RW' },
  { id: 'A11', name: 'Erling Haaland', number: 9, team: 'A', x: 80,  y: 30, pmu: 45, position: 'ST' },
  ...
  { id: 'B3', name: 'Victor Lindelof', number: 2, team: 'B', x: 85,  y: 28, pmu: 25, position: 'CB' },
  { id: 'B4', name: 'Raphael Varane', number: 19, team: 'B', x: 85,  y: 40, pmu: 24, position: 'CB' },
  { id: 'B5', name: 'Aaron Wan-Bissaka', number: 29, team: 'B', x: 83,  y: 56, pmu: 21, position: 'RB' },
  { id: 'B6', name: 'Scott McTominay', number: 39, team: 'B', x: 67,  y: 34, pmu: 28, position: 'DM' },
  { id: 'B7', name: 'Bruno Fernandes', number: 18, team: 'B', x: 57,  y: 22, pmu: 30, position: 'CM' },
  { id: 'B8', name: 'Christian Eriksen', number: 14, team: 'B', x: 57,  y: 46, pmu: 29, position: 'CM' },
  { id: 'B9', name: 'Antony Silva', number: 21, team: 'B', x: 38,  y: 20, pmu: 35, position: 'FW' },
  { id: 'B10', name: 'Marcus Rashford', number: 10, team: 'B', x: 38,  y: 48, pmu: 33, position: 'FW' },
]
...
export default function Pitch({ players = [], teamAPressure = {}, teamBPressure = {}, showHeat = true }) {
  const roster = players?.length ? players : DEMO_PLAYERS

  /* top-3 per team for heatspot rendering */
  const topA = [...roster.filter(p => p.team === 'A')].sort((a, b) => (b.pmu || 0) - (a.pmu || 0)).slice(0, 3)
  const topB = [...roster.filter(p => p.team === 'B')].sort((a, b) => (b.pmu || 0) - (a.pmu || 0)).slice(0, 3)
  ...
  return (
    <div className="pitch-card">
      ...
      >
        ...
        {/* ── Pitch markings ── */}
        {/* Boundary */}
        <rect x="0" y="0" width={FW} height={FH} fill="none"
          stroke="rgba(255,255,255,0.28)" strokeWidth="0.45" />
        ...
        {/* Centre circle */}
        <circle cx={FW/2} cy={FH/2} r="9.15"
          stroke="rgba(255,255,255,0.22)" strokeWidth="0.35" fill="none" />

        {/* Centre spot */}
        <circle cx={FW/2} cy={FH/2} r="0.45" fill="rgba(255,255,255,0.6)" />

        {/* Penalty areas */}
        <rect x="0"         y={(FH/2)-20.15} width="16.5" height="40.3"
          stroke="rgba(255,255,255,0.22)" strokeWidth="0.35" fill="none" />
        <rect x={FW-16.5}   y={(FH/2)-20.15} width="16.5" height="40.3"
          stroke="rgba(255,255,255,0.22)" strokeWidth="0.35" fill="none" />

        {/* 6-yard boxes */}
        <rect x="0"       y={(FH/2)-9} width="5.5" height="18"
          stroke="rgba(255,255,255,0.18)" strokeWidth="0.3" fill="none" />
        <rect x={FW-5.5}  y={(FH/2)-9} width="5.5" height="18"
          stroke="rgba(255,255,255,0.18)" strokeWidth="0.3" fill="none" />
        ...
        {/* Goal lines */}
        <line x1="0" y1={FH/2-3.66} x2="-1.5" y2={FH/2-3.66}
          stroke="rgba(255,255,255,0.5)" strokeWidth="0.5" />
        <line x1="0" y1={FH/2+3.66} x2="-1.5" y2={FH/2+3.66}
          stroke="rgba(255,255,255,0.5)" strokeWidth="0.5" />
        <line x1={FW} y1={FH/2-3.66} x2={FW+1.5} y2={FH/2-3.66}
          stroke="rgba(255,255,255,0.5)" strokeWidth="0.5" />
        <line x1={FW} y1={FH/2+3.66} x2={FW+1.5} y2={FH/2+3.66}
          stroke="rgba(255,255,255,0.5)" strokeWidth="0.5" />

        {/* ── Pressure heatspots ── */}
        {showHeat && hotspots.map(h => (
          <circle
            key={`hs-${h.id}`}
            cx={h.x} cy={h.y}
            r={hR(h.pmu)}
        ...
        {roster.map(p => {
          ...
          return (
            ...
            >
              ...
              <title>{`${p.name} #${p.number} [${p.team}] PMU ${p.pmu?.toFixed ? p.pmu.toFixed(1) : '—'}`}</title>
            </g>
          )
        })}
        ...
        {/* ── Hover tooltip ── */}
        {hovered && (
          <g transform={`translate(${Math.min(hovered.x + 3, FW - 20)},${Math.max(hovered.y - 8, 2)})`}>
            <rect x="0" y="0" width="26" height="11" rx="1.5"
              fill="rgba(13,18,32,0.9)" stroke="rgba(0,229,160,0.4)" strokeWidth="0.3" />
            <text x="1.5" y="3.8" fontSize="2.6" fill="#eef2ff" fontWeight="700"
              style={{ fontFamily: 'Inter, sans-serif', pointerEvents: 'none' }}>
              {shortenPlayerName(hovered.name, 'medium')} #{hovered.number}
            </text>
            <text x="1.5" y="7.2" fontSize="2.0" fill="rgba(120,140,180,0.9)"
              style={{ fontFamily: 'Inter, sans-serif', pointerEvents: 'none' }}>
      ...
      <div className="pitch-legend">
        <div className="legend-item">
          <div className="legend-dot" style={{ background: A_COL, boxShadow: `0 0 6px ${A_COL}` }} />
          Team A
        </div>
        <div className="legend-item">
          <div className="legend-dot" style={{ background: B_COL, boxShadow: `0 0 6px ${B_COL}` }} />
          Team B
        </div>
        <div className="legend-item">
          <div className="legend-dot" style={{
            background: 'radial-gradient(circle, rgba(59,130,246,0.6) 0%, rgba(244,114,182,0.6) 100%)',
          }} />
          Pressure zone
        </div>
      </div>
