  ...
  const notifications = useMemo(() => {
    ...
    if ((simResults.xg || 0) > 2.5) {
      notifs.push({
        ...
        message: `Expected goals ratio is elevated at ${(simResults.xg || 0).toFixed(
          2,
        )}. Both teams showing strong attacking potential.`,
        timestamp: new Date(),
      });
    }
    ...
    if (simResults.avgPMU_A && simResults.avgPMU_B) {
      ...
      if (momentumDiff > 15) {
        notifs.push({
          ...
          message: `${leader} has a ${momentumDiff.toFixed(
            1,
          )} PMU advantage. Strong positioning for control.`,
          timestamp: new Date(),
        });
      }
    }
    ...
    if (simResults.avgPossession) {
      ...
      if (poss > 65 || poss < 35) {
        ...
        notifs.push({
          ...
          message: `${team} showing ${level} ball possession at ${poss.toFixed(
            1,
          )}%. Watch for counter-attack opportunities.`,
          timestamp: new Date(),
        });
      }
    }
    ...
    if (simResults.avgTeamFatigue && simResults.avgTeamFatigue > 75) {
      notifs.push({
        ...
        message: `Average team fatigue at ${simResults.avgTeamFatigue.toFixed(
          1,
        )}%. Performance degradation likely in late minutes.`,
        timestamp: new Date(),
      });
    }
    ...
    if (simResults.crowdNoise && simResults.crowdNoise > 85) {
      notifs.push({
        ...
        message: `Crowd noise at ${simResults.crowdNoise.toFixed(
          0,
        )}%. Significant psychological advantage for home team.`,
        timestamp: new Date(),
      });
    }
    ...
    if (simResults.goals_a !== undefined && simResults.goals_b !== undefined) {
      ...
      if (diff >= 2) {
        ...
        notifs.push({
          ...
          message: `${team} leads ${Math.max(simResults.goals_a, simResults.goals_b)}-${Math.min(
            simResults.goals_a,
            simResults.goals_b,
