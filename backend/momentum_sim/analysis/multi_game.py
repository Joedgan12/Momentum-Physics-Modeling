...
class MultiGameAnalyzer:
    ...
    def aggregate_player_momentum(self, player_id: str, position: str = None) -> Dict:
        ...
        return {
            ...
            "consistency": float(
                1 - (np.std(pmuls) / (np.mean(pmuls) + 1e-6))
            ),  # 0-1, higher = consistent
        }
    ...
    def aggregate_team_momentum(self, team_id: str) -> Dict:
        ...
        formation_coherences = (
            np.array(formation_coherences) if formation_coherences else np.array([0.8])
        )
        ...
        return {
            ...
            "momentum_stability": float(
                1 - (np.std(team_momentums) / (np.mean(team_momentums) + 1e-6))
            ),
            "avg_formation_coherence": float(np.mean(formation_coherences)),
            "matches_analyzed": len(self.matches),
        }

    def identify_tactical_zones(
        self, team_id: str, min_momentum_threshold: float = 5.0
    ) -> Dict:
        """
        ...
        for zone, momentums in zone_momentums.items():
            if momentums:
                ...
                stats[zone] = {
                    ...
                    "impact": float(
                        np.mean(momentums) * len(momentums)
                    ),  # Cumulative impact
                }
            else:
                stats[zone] = {
    ...
    def detect_undervalued_players(
        self, position: str = None, market_value_threshold: float = None
    ) -> List[Dict]:
        """
                        ...
                        players_data[p["id"]].append(
                            {
                                ...
                                "pmu_stats": self.aggregate_player_momentum(
                                    p["id"], position
                                ),
                            }
                        )
    ...
    def formation_analysis(self, team_id: str, formation: str) -> Dict:
        ...
        return {
            ...
            "goal_differential": float(np.mean(goals_for) - np.mean(goals_against))
...
class TacticalFingerprint:
    ...
    def compute_pressure_profile(team_data: List[Dict]) -> Dict:
        ...
        return {zone: float(np.mean(values)) for zone, values in pressure_zones.items()}
