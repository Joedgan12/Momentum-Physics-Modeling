...
class PMUValidator:
    ...
    def decay_curve_validation(
        event_pmu_trace: List[Tuple[float, float]],
        resilience_type: str = 'veteran'
    ) -> Dict:
        """
    ...
    def counterfactual_validation(
        historical_match_outcomes: List[Dict],
        simulated_scenarios: List[Dict]
    ) -> float:
        """
        ...
        historical_outcomes = np.array([m['final_momentum'] for m in historical_match_outcomes])
        simulated_outcomes = np.array([s.get('final_momentum', 50) for s in simulated_scenarios])
        ...
        plausibility = within_range / len(simulated_scenarios) if simulated_scenarios else 0
        return float(plausibility)
    
    @staticmethod
    def formation_coherence_validation(
        predicted_coherence: List[float],
        actual_defensive_success: List[int]
    ) -> float:
        """
