...
class EventProcessor:
    ...
    def compute(
        ...
        zone_factor = ZONE_MODS.get(
            EventProcessor.zone_from_x(player.x, player.team), 1.0
        )
...
class DecayModel:
    ...
    def apply(player: PlayerState, event_type: str = "default", dt: float = 1.0):
        ...
        if event_type == "goal_conceded":
            # Exponential shock
            decay_amount = abs(player.event_impact) * (
                1.0 - math.exp(-GOAL_CONCEDED_LAMBDA * dt)
            )
        else:
            decay_amount = rate * dt

        player.event_impact = max(
            0.0, player.event_impact * player.resilience - decay_amount
        )
        player.recalc_pmu()
...
class PressureEngine:
    ...
    def cone_factor(
        ...
        tx: float,
        ty: float,
        cone_deg: float = PRESSURE_CONE_DEG,
    ) -> float:
        """
    ...
    def compute_impact(
        ...
        c_factor = PressureEngine.cone_factor(
            pressurer.x,
            pressurer.y,
            target.x - pressurer.x,
            target.y - pressurer.y,  # facing toward target
            target.x,
            target.y,
        )
...
class AgentDecision:
    ...
    def attempt_action(
        ...
        prob = base * AgentDecision._skill_factor(
            player
        ) - AgentDecision._fatigue_penalty(player)
        ...
        resolved = (
            event_map_success.get(action, action)
            if success
            else event_map_fail.get(action, action)
        )
...
class MatchSimulator:
    ...
    def _run_step(self, minute: int):
        ...
        for player in self._all_players():
            gs = (
                self.match_state.game_state_a
                if player.team == "A"
                else self.match_state.game_state_b
            )
                    ...
                    opp.event_log.append(
                        {
                            "minute": minute,
                            "event": "goal_conceded",
                            "impact": round(concede_impact * conc_mod, 2),
                            "success": False,
                        }
                    )
            ...
            if total_pressure > 0:
                player.event_impact -= (
                    total_pressure * 0.05
                )  # 5% of pressure converts to PMU loss
                player.recalc_pmu()
    ...
    def _aggregate(self, results: List[Dict]) -> Dict:
            ...
            for p in r.get("allPlayers", []):
                ...
                if pid not in player_pmu_acc:
                    ...
                    player_meta[pid] = {
                        ...
                        "position": p["position"],
                        "team": p["team"],
                        "resilience_tier": p.get("resilience_tier", ""),
                    }
        ...
        return {
            ...
            "scoreDistribution": {
                ...
                "std_goals_a": round(
                    statistics.stdev(goals_a) if len(goals_a) > 1 else 0.0, 2
                ),
                "std_goals_b": round(
                    statistics.stdev(goals_b) if len(goals_b) > 1 else 0.0, 2
                ),
            },
            "goalProbDistribution": gp_bins,
            "teamAPressure": {
                "possession": round(
                    sum(r["teamAPressure"]["possession"] for r in results) / N, 4
                ),
                "offBall": round(
                    sum(r["teamAPressure"]["offBall"] for r in results) / N, 4
                ),
                "transition": round(
                    sum(r["teamAPressure"]["transition"] for r in results) / N, 4
                ),
            },
            "teamBPressure": {
                "possession": round(
                    sum(r["teamBPressure"]["possession"] for r in results) / N, 4
                ),
                "offBall": round(
                    sum(r["teamBPressure"]["offBall"] for r in results) / N, 4
                ),
                "transition": round(
                    sum(r["teamBPressure"]["transition"] for r in results) / N, 4
                ),
            },
