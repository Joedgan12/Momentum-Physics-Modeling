...
class DecayModel:
    ...
    def get_decay_rate(event_type: EventType) -> float:
        ...
        return DecayModel.DECAY_RATES.get(event_type, DecayModel.DECAY_RATES[EventType.DEFAULT])
    
    @staticmethod
    def exponential_decay(initial_pmu: float, time_elapsed: float, lambda_param: float = None) -> float:
        """
    ...
    @staticmethod
    def linear_decay(initial_pmu: float, time_elapsed: float, decay_rate: float) -> float:
        """
