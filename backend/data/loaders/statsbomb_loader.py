...
class StatsBombLoader:
    ...
    def extract_match_stats(self, match: Dict) -> Dict:
        ...
        passes_a = sum(1 for e in events if e.get("type") == "Pass" and 
                      e.get("team", {}).get("id") == home_team.get("home_team_id"))
        passes_b = sum(1 for e in events if e.get("type") == "Pass" and 
                      e.get("team", {}).get("id") == away_team.get("away_team_id"))
        
        # Count tackles (proxy for pressure)
        tackles_a = sum(1 for e in events if e.get("type") == "Tackle" and 
                       e.get("team", {}).get("id") == home_team.get("home_team_id"))
        tackles_b = sum(1 for e in events if e.get("type") == "Tackle" and 
                       e.get("team", {}).get("id") == away_team.get("away_team_id"))
        
        # Count shots
        shots_a = sum(1 for e in events if e.get("type") == "Shot" and 
                     e.get("team", {}).get("id") == home_team.get("home_team_id"))
        shots_b = sum(1 for e in events if e.get("type") == "Shot" and 
                     e.get("team", {}).get("id") == away_team.get("away_team_id"))
        ...
        return {
            ...
            "shots_a": shots_a,
            "shots_b": shots_b,
            "possession_a": round(passes_a / (passes_a + passes_b) * 100, 1) if (passes_a + passes_b) > 0 else 50.0,
        }
