...
class SyntheticDatasetGenerator:
    ...
    TEAM_NAMES = [
        "Arsenal", "Liverpool", "Manchester United", "Manchester City", "Chelsea",
        "Tottenham", "Newcastle", "Brighton", "Aston Villa", "Fulham",
        "Brentford", "Everton", "Ipswich", "Nottingham", "Bournemouth",
    ]
    ...
    def generate_match(cls, match_id: int) -> Dict:
        ...
        goals_a += 1 if random.random() < (goal_prob_a * 0.3) else 0  # 30% chance of 2nd goal
    ...
    @staticmethod
    def save_dataset(matches: List[Dict], output_path: str = "backend/data/synthetic_matches.json"):
...
if __name__ == "__main__":
    ...
    print(f"  Avg goals (Team A): {sum(m['goals_a'] for m in dataset) / len(dataset):.2f}")
    print(f"  Avg goals (Team B): {sum(m['goals_b'] for m in dataset) / len(dataset):.2f}")
